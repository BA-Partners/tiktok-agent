...
import { augmentMessagesWithRag } from '../rag/local.mjs';
...
export async function createChatCompletion(requestBody = {}) {
  ...
  const { messages: augmentedMessages, rag } = augmentMessagesWithRag(messages, {
    rag: requestBody.rag,
    topK: requestBody.rag_top_k
  });
  ...
  const promptTokens = estimateMessageTokens(augmentedMessages);
  ...
  return {
    ...
    rag: {
      enabled: rag.enabled,
      sources: rag.sources.map(({ id, source, chunk, score }) => ({ id, source, chunk, score }))
    },
    choices: [
      {
        index: 0,
